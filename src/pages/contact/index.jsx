...
const Index = () => {
	...
	const validateField = (name, value) => {
		if (value.trim() && VALIDATION_PATTERNS[name]) {
			...
			// If empty, clear error (or set required error if we wanted aggressive validation)
			// For now, we just clear specific validation errors
			setErrors(prev => {
				const newErrors = { ...prev }
				delete newErrors[name]
				return newErrors
	...
	const handleChange = (e) => {
		...
		if (name === 'email' || name === 'name') {
			...
			// For other fields, just clear errors immediately if they have value
